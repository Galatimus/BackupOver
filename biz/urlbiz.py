...
try:
    os.remove('/home/oleg/pars/biz/cian_biz.txt')
    print 'Удаляем: '
...
while i < len(ls):
	...
	def task_post(self,grab,task):
	    for elem in grab.doc.select(u'//a[contains(@href,"sale/commercial")]'):
                url = grab.make_url_absolute(elem.attr('href')) 
                lin.append(url)
	    for elem in grab.doc.select(u'//a[contains(@href,"rent/commercial")]'):
	        url = grab.make_url_absolute(elem.attr('href'))   
	        lin.append(url) 
...
print('Done')
time.sleep(5)
os.system("/home/oleg/pars/biz/cian.py")
