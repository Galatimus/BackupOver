...
i = 0
l= open('links/kv1.txt').read().decode('cp1251').splitlines()
dc = len(l)
page = l[i] 
oper = u'Продажа'
	  ...
	  def prepare(self):
	       ...
	       while True:
		    try:
			 ...
			 g = Grab(timeout=20, connect_timeout=20)
			 g.go(self.f)
			 self.sub = g.doc.select(u'//span[@class="current"]').text().replace(u'Кызыл',u'Республика Тыва').replace(u'Долгопрудный',u'Московская область').replace(u'Гатчина',u'Ленинградская область')
			 try:
	       ...
	       self.workbook = xlsxwriter.Workbook(u'Mirkvartir_%s' % self.sub + u'_Жилье_'+oper+'.xlsx')
		 ...
		 #print rayon
	       ...
	       try:
		    ...
		    conv = [ (u'сегодня', (datetime.today().strftime('%d.%m.%Y'))),
		         ...
		         (u'августа', '.08.2016'),(u'мая', '.05.2017'),(u'ноября', '.11.2016'),
		         (u'марта', '.03.2017'),(u'сентября', '.09.2016'),(u'октября', '.10.2016'),(u'января', '.01.2017'),(u'февраля', '.02.2017'),(u'апреля', '.04.2017'),
		         (u'июля', '.07.2016'),(u'июня', '.06.2017'),(u'декабря', '.12.2016')]
