...
name =open('urls.txt').read().splitlines()
for li in name:
       nums=len(name)       
class Gis(Spider):
       ...
       def prepare(self):
              self.workbook = xlsxwriter.Workbook(u'Актуальность_'+datetime.today().strftime('%d.%m.%Y')+'.xlsx')
              self.ws = self.workbook.add_worksheet()
              self.ws.write(0,0, u"URL")
              self.ws.write(0,1, u"Актуальность")
              self.row= 1  
       ...
       def task_post(self,grab,task):
              ...
              elif 'move.ru/objects' in task.url:
