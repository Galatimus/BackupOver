...
i = 5
ls= open('zem.txt').read().splitlines()
dc = len(ls)
...
while i < len(ls):
           ...
           sub = driver.find_element_by_xpath(u'//span[@class="glyphicon glyphicon-map-marker"]/following::input[1]').get_attribute('value').replace(u'город ','')
           print sub
           #time.sleep(1)   
           ...
           workbook = xlsxwriter.Workbook(u'zem/Kvmeter_%s' % sub + u'_Земля_'+oper+str(i+1)+'.xlsx')
           ...
           while True:
                      ...
                      try:
                                 try:
                                            ...
                                            break
                                 time.sleep(1)
                                 for link in driver.find_elements_by_xpath(u'//a[contains(@href,"objects")]'):
                                            url = link.get_attribute('href')   
                                            print url
                                            lin.append(url)
                                 time.sleep(1)   
                                 driver.execute_script("arguments[0].scrollIntoView(false);",driver.find_element_by_xpath(u'//div[@class="b-navBar clearfix"]'))                                 
                                 time.sleep(1)
                                 driver.find_element_by_xpath(u'//li[@class="active"]/following-sibling::li/a[contains(text(),"Следующая»")]').click()
                                 time.sleep(2)
                                 ...
                                 for line in lin:
                                            ...
                                            try:
                                                       ray = driver.find_element_by_xpath(u'//p[@class="pbig"]/a[contains(@href,"rajon")][1][contains(text(),"район")]').text
                                            ...
                                            ws.write(result, 23, u'Метр квадратный')
