...
workbook = xlsxwriter.Workbook(u'com/Raui_Коммерческая_Продажа.xlsx')
...
class Farpost_Com(Spider):
     def prepare(self):
	  ...
	  for p in range(1,5):
	       try:
		    ...
		    g.go('https://youla.ru/cities')
		    print g.doc.code
		    if g.doc.code ==200:
			 for elem in g.doc.select(u'//li[@class="cities_list__item"]/a'):
                              ur = g.make_url_absolute(elem.attr('href'))  
                              print ur
			      links = open('youla.txt', 'a')
			      links.write("%s\n" % ur)
			      links.close()      
			 del g
			 break
     ...
     def task_generator(self):
	  for x1 in range(1,int(self.num)+1):
	       yield Task ('post',url='https://raui.ru/kupit-office-sklad?page=%d'%x1,refresh_cache=True,network_try_count=100)
