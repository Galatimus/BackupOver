...
i = 0
l= open('Links/kv1.txt').read().splitlines()
dc = len(l)
page = l[i]  
oper = u'Продажа'
          ...
          def prepare(self):
	       ...
	       while True:
		    try:
			 ...
			 self.sub = reduce(lambda dt, r: dt.replace(r[0], r[1]), conv, dt).replace(u'Кызыле',u'Республика Тыва').replace(u'в Долгопрудном',u'Московская область').replace(u'в Гатчине',u'Ленинградская область')
			 ...
			 g.change_proxy()
			 continue	  	       
	       self.workbook = xlsxwriter.Workbook(u'Dmir_%s' % bot.sub + u'_Жилье_'+oper+'.xlsx')
