...
main_grp.add_option('--input-file',default = 'urls.txt', nargs = 1)
...
main_grp.add_option('--workers',default = 3, nargs = 1)
...
#conn_grp.add_option('-t', '--timeout', default = 20, nargs = 1)
...
def shell_exec(url, command, options):
    ...
    timeout = 10 #int(options.timeout)
...
def craft_cmd(url_and_options):
    ...
    #options.proxy = random.choice(list(open('../../../tipa.txt').read().splitlines()))
    ...
    cmd_parameters = ['phantomjs --ignore-ssl-errors true','--ssl-protocol any','--ssl-ciphers ALL']
