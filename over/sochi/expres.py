...
workbook = xlsxwriter.Workbook(u'Sochi_express.xlsx') 
...
class Cian_Kv(Spider):
     ...
     def task_generator(self):
	  for x in range(1,215):#230
               yield Task ('post',url='http://www.sochi-express.ru/realty/sale/secondary/?page=%d'%x,network_try_count=100)
          for x1 in range(1,36):#92
               yield Task ('post',url='http://www.sochi-express.ru/realty/sale/new/?page=%d'%x1,network_try_count=100)
     ...
     def task_item(self, grab, task):
	  ...
	  try:
	       dom = re.sub('[^\d]','',grab.doc.select(u'.//*[@id="wrap"]/div[2]/div[1]/div[2]/div[2]/div[1]/div[1]/p[2]/text()[1]').text())
	   #print rayon
	  except DataNotFound:
	       dom = ''
	  ...
	  try:
	       novo = grab.doc.select(u'.//*[@id="wrap"]/div[2]/div[1]/div[2]/div[2]/div[1]/div[1]/p[1]/text()[1]').text().replace(u'Вторичное жилье',u'Нет').replace(u'Новостройки',u'Да')
	  except DataNotFound:
	       novo = ''
          try:
          ...
          try:
               ...
               price = ''
          try:
               opis = grab.doc.select(u'//div[@class="description"]').text().replace(u'Количество просмотров: ','') 
          except DataNotFound:
               opis = ''	       
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write(self.result, 11, u'Sochi-Express.ru')
