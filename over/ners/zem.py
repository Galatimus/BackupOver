...
while True:
     ...
     class Ners_zem(Spider):
	  def prepare(self):
	       ...
	       self.link =l[i]
			 ...
			 g.proxylist.load_file(path='../../tipa.txt',proxy_type='http')			 
                         g.go(self.f)
                         self.num = re.sub('[^\d]','',g.doc.select(u'//div[@class="title_count"]').text())
			 self.pag = int(math.ceil(float(int(self.num))/float(15)))
			 s = g.doc.select(u'//ul[@class="linklist navlinks"]/li[1]/a').text()
			 if u'Недвижимость' in s:
			      self.sub = g.doc.select(u'//ul[@class="linklist navlinks"]/li[2]/a').text()
			 else:
			      self.sub = s
	  ...
	  def task_generator(self):
	       for x in range(1,self.pag+1):
                    yield Task ('post',url=self.f+'?start='+str(x*15),refresh_cache=True,network_try_count=10)
	  ...
	  def task_post(self,grab,task):
	       ...
	       for elem in grab.doc.select('//h2/a[contains(@href,"object")]'):
		    ...
		    it.setup(url=ur,headers=headers)
	  ...
	  def task_item(self, grab, task):
	       ...
	       try:
		    key = re.sub('[^\d]','',grab.doc.select(u'//div[@class="notes_id"]/b').text()) 
                    user = re.sub('[^\d]','',grab.doc.select(u'//div[@id="get_phone"]/@data-u').text())
                    pkey = re.sub('[^\d]','',grab.doc.select(u'//body/@data-stat-id').text())
                    link = task.url.split(u'object')[0]#+u'ru'
		    print link
                    url_ph = link+'ajax/?module=notes_get_phone&notes_id='+key+'&user_id='+user+'&stat_id='+pkey+'&app_alias=ru&json=1'
                    headers ={'Accept': 'application/json, text/javascript, */*; q=0.01',
		              'Accept-Encoding': 'gzip,deflate',
		              'Accept-Language': 'ru-RU,ru;q=0.8,en-US;q=0.5,en;q=0.3',
		    ...
		    payload = (('app_alias', 'ru'), ('db_importer_id', 0),('is_my_notes', 0),('json',1), ('module', 'notes_get_phone'),('notes_id',key),('stat_id', pkey), ('user_id', user))
		    r= requests.post(url_ph,verify=True,allow_redirects=False,headers=headers,data=payload,timeout=15000) 
		    print r.text
     ...
     bot = Ners_zem(thread_number=3, network_try_limit=100)
