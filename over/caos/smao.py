...
workbook = xlsxwriter.Workbook(u'Smao.xlsx')
...
class Cian_Zem(Spider):
     ...
     def task_generator(self):
	  for x in range(79):
               yield Task ('post',url='http://lko.smao.ru/registry?search=&skip=%d'%x,network_try_count=100)
	       
	       
     def task_post(self,grab,task):
	  for elem in grab.doc.select('//a[contains(@href,"id")]'):
     ...
     def task_item(self, grab, task):
	  ...
	  try:
	       ...
	       ray = ''          
	  try:
	       punkt= grab.doc.select(u'//tr[@class="title"]/td[contains(text(),"Организация")]/following::tr[1]/td[1]/text()[1]').text()#.split('Должность: ')[1].split(' Подробнее')[0]
	       if 'ОРГНИП' in punkt:
		    punkt = ''
	       else:
		    punkt = punkt
	  ...
	  try:
	       ...
	       #for m in grab.doc.select(u'//td[contains(text(),"Квалификационный аттестат")]/following-sibling::td/b[contains(text(),"Направление:")]/following-sibling::text()[1]'):
		    ...
		    #ln.append(mes)
	       #ter = ', '.join(ln)
	       ter= grab.doc.select(u'//tr[@class="title"]/td[contains(text(),"Вид образования")]/following::tr[2]/td[2]').text()
	  except IndexError:
	       ter =''
	  ...
	  try:
	       ...
	       uliza = ''
	  try:
	       dom = grab.doc.select(u'//td[contains(text(),"Номера контактных телефонов")]/following-sibling::td/div').text()#.split('Контактный телефон: ')[1].split('Изменено')[0]
	  except IndexError:
	       dom = ''       
	  try:
	       trassa = grab.doc.select(u'//td[contains(text(),"Адрес электронной почты")]/following-sibling::td/div').text()#.split('E-mail организации: ')[1].split('Адрес')[0]
	  except IndexError:
	       trassa = ''       
	  try:
	       udal = grab.doc.select(u'//td[contains(text(),"Почтовый адрес")]/following-sibling::td/div').text()#.split(' г. ')[0]
	  except IndexError:
	       udal = ''
	       
	  try:
	       price = grab.doc.select(u'//td[contains(text(),"Статус/Соответствие требованиям к членству")]/following-sibling::td').text()#.split(u'Цена - ')[1]
	  except IndexError:
	       price = ''   
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write(self.result, 11, u'Реестр СМАО')
