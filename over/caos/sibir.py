...
def take_content(driver, url):
    ...
    time.sleep(3)
    try:
        zag = driver.find_element_by_xpath(u'//h3').text
    except (NoSuchElementException,WebDriverException):
        zag = ''
    try:
        uliza = driver.find_element_by_xpath('//h4[contains(text(),"Информация об отчётах")]/following::table/tbody/tr[1]/td[5]').text
    except (NoSuchElementException,IndexError,WebDriverException):
        uliza = ''
    try:
        ray = driver.find_element_by_xpath('//h4[contains(text(),"Образование")]/following::table/tbody/tr[2]/td[4]').text
    except (NoSuchElementException,IndexError,WebDriverException):
        ray = ''
    try:
        punkt = driver.find_element_by_xpath('//td[contains(text(),"Контактный телефон")]/following-sibling::td').text
    except (NoSuchElementException,IndexError,WebDriverException):
        punkt =''
    try:
        cena = driver.find_element_by_xpath('//td[contains(text(),"E-Mail")]/following-sibling::td').text
    except (NoSuchElementException,IndexError,WebDriverException):
        cena = ''
    try:
        oren = driver.find_element_by_xpath('//td[contains(text(),"Регионы деятельности")]/following-sibling::td').text
    except (NoSuchElementException,IndexError,WebDriverException):
        oren = ''
    try:
        seg = driver.find_element_by_xpath('//td[contains(text(),"Статус")]/following-sibling::td').text
    except (NoSuchElementException,IndexError,WebDriverException):
        seg =''
    try:
    ...
    ws.write(result, 8, oren)
    ws.write(result, 6, cena)
    ws.write(result, 11, u'НП СОО "СИБИРЬ"')
    ws.write(result, 12, datetime.today().strftime('%d.%m.%Y'))
    ws.write(result, 10, seg)
...
if __name__ == '__main__':
    l= open('reestr1.txt').read().splitlines()
    workbook = xlsxwriter.Workbook(u'Soosibir.xlsx')    
