...
workbook = xlsxwriter.Workbook(u'Sromso.xlsx')
...
class Cian_Zem(Spider):
     ...
     def task_generator(self):
	  ...
	  for x in range(75):
	       ...
	       if x > 70:
		    z=7	  
               yield Task ('post',url='http://www.sromso.ru/membershipMSO/reestr/v2/index.php?page=%d'%x+'&section=%d'%z,network_try_count=100)
     ...
     def task_post(self,grab,task):
	  for elem in grab.doc.select('//td[@scope="row"]/a'):
     ...
     def task_item(self, grab, task):
	  ...
	  try:
	       ...
	       ray = ''          
	  try:
	       punkt= grab.doc.select(u'//caption[contains(text(),"Сведения о юридическом лице, с которым оценщик заключил трудовой договор:")]/following-sibling::thead/following-sibling::tbody/tr[1]/td[1]').text()#.split('Должность: ')[1].split(' Подробнее')[0]
	  except IndexError:
	       punkt = ''
	  ...
	  try:
	       ...
	       #for m in grab.doc.select(u'//td[contains(text(),"Квалификационный аттестат")]/following-sibling::td/b[contains(text(),"Направление:")]/following-sibling::text()[1]'):
		    ...
		    #ln.append(mes)
	       #ter = ', '.join(ln)
	       ter= grab.doc.select(u'//caption[contains(text(),"Информация о квалификационном аттестате")]/following-sibling::thead/following-sibling::tbody/tr[1]/td[1]').text()
	  except IndexError:
	       ter =''
	  ...
	  try:
	       ...
	       uliza = ''
	  try:
	       dom = grab.doc.select(u'//th[contains(text(),"Контактная информация")]/following::tr[1]/td[2]').text().split(', ')[0]
	  except IndexError:
	       dom = ''       
	  try:
	       trassa = grab.doc.select(u'//th[contains(text(),"Контактная информация")]/following::tr[1]/td[2]').text().split(', ')[1]
	  except IndexError:
	       trassa = ''       
	  try:
	       udal = grab.doc.select(u'//th[contains(text(),"Контактная информация")]/following::tr[2]/td[2]').text().split(', ')[1].replace(u'г. ','')
	  except IndexError:
	       udal = ''
	       
	  try:
	       price = grab.doc.select(u'//td[contains(text(),"Статус")]/following::b[1]').text()#.split(u'Цена - ')[1]
	  except IndexError:
	       price = ''   
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write_string(self.result, 7, task.project['trassa'])
	  self.ws.write(self.result, 8, task.project['udal'])
	  self.ws.write(self.result, 10, task.project['cena'])
	  self.ws.write(self.result, 11, u'Ассоциация «МСО»')
