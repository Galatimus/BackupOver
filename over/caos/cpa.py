...
workbook = xlsxwriter.Workbook(u'Cpa-Russia.xlsx')
...
class Cian_Zem(Spider):
     ...
     def task_generator(self):
	  for x in range(1,31):
               yield Task ('post',url='https://reestr.cpa-russia.org/site/index?page=%d'%x,network_try_count=100)
     ...
     def task_item(self, grab, task):
	  try:
	       sub = grab.doc.select(u'//h3[@class="col-md-6"]').text()
	  except IndexError:
	       sub = ''
	  try:
	       ...
	       ray = ''          
	  try:
	       punkt= grab.doc.select(u'//b[contains(text(),"Наименование")]/following-sibling::text()').text()#.split('Должность: ')[1].split(' Подробнее')[0]
	  except IndexError:
	       punkt = ''
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write(self.result, 11, u'НП СРОО «СПО»')
