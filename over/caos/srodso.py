...
workbook = xlsxwriter.Workbook(u'Srodso.xlsx')
...
class Cian_Zem(Spider):
     ...
     def task_generator(self):
	  #for x in range(1,28):
          yield Task ('post',url='http://www.srodso.ru/01.01.03.01/member_list.aspx',network_try_count=100)
     ...
     def task_post(self,grab,task):
	  for elem in grab.doc.select('//a[contains(@href,"member_card")]'):
     ...
     def task_item(self, grab, task):
	  ...
	  try:
	       ...
	       ray = ''          
	  try:
	       punkt= grab.doc.select(u'//table[@id="ctl00_pContent_gv_job"]/tr/td').text()#.split('Должность: ')[1].split(' Подробнее')[0]
	  except IndexError:
	       punkt = ''
	  ...
	  try:
	       ...
	       #for m in grab.doc.select(u'//td[contains(text(),"Квалификационный аттестат")]/following-sibling::td/b[contains(text(),"Направление:")]/following-sibling::text()[1]'):
		    ...
		    #ln.append(mes)
	       #ter = ', '.join(ln)
	       ter= grab.doc.select(u'//th[contains(text(),"Направление ОД")]/following::tr[1]/td[5]').text()
	  except IndexError:
	       ter =''
	  ...
	  try:
	       ...
	       uliza = ''
	  try:
	       dom = grab.doc.select(u'//span[@id="ctl00_pContent_txt_phone"]').text()#.split(', ')[0]
	  except IndexError:
	       dom = ''       
	  try:
	       trassa = grab.doc.select(u'//span[@id="ctl00_pContent_txt_email"]').text()#.split(', ')[1]
	  except IndexError:
	       trassa = ''       
	  try:
	       udal = grab.doc.select(u'//span[@id="ctl00_pContent_txt_regions"]').text()#.split(', ')[1].replace(u'г.','')
	  except IndexError:
	       udal = ''
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write(self.result, 11, u'НП СРО «ДСО»')
