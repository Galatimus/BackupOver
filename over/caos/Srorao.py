...
workbook = xlsxwriter.Workbook(u'Srorao.xlsx')
...
class Cian_Zem(Spider):
     ...
     def task_generator(self):
	  #for x in range(1,28):
          yield Task ('post',url='http://api.srorao.ru/api/app/member_list_app.aspx',refresh_cache=True,network_try_count=100)
     ...
     def task_item(self, grab, task):
	  ...
	  try:
	       ...
	       ray = ''          
	  try:
	       punkt= grab.doc.select(u'//table[@id="ctl00_ContentPlaceHolder1_gv_job"]/tr/td').text().replace(u'Стаж в оценочной деятельности','')#.split('Должность: ')[1].split(' Подробнее')[0]
	  except IndexError:
	       punkt = ''
	  ...
	  try:
	       ...
	       #for m in grab.doc.select(u'//td[contains(text(),"Квалификационный аттестат")]/following-sibling::td/b[contains(text(),"Направление:")]/following-sibling::text()[1]'):
		    ...
		    #ln.append(mes)
	       #ter = ', '.join(ln)
	       ter= grab.doc.select(u'//th[contains(text(),"Направление")]/following::tr[1]/td[2]').text()
	  except IndexError:
	       ter =''
	  ...
	  try:
	       ...
	       uliza = ''
	  try:
	       dom = grab.doc.select(u'//th[contains(text(),"Контактный телефон")]/following::tr[1]/td[2]').text()#.split(', ')[0]
	  except IndexError:
	       dom = ''       
	  try:
	       trassa = grab.doc.select(u'//th[contains(text(),"Email")]/following::tr[1]/td[3]').text()#.split(', ')[1]
	  except IndexError:
	       trassa = ''       
	  try:
	       udal = grab.doc.select(u'//th[contains(text(),"Регионы деятельности")]/following-sibling::td').text()#.split(', ')[1].replace(u'г.','')
	  except IndexError:
	       udal = ''
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write(self.result, 11, u'СРО РАО')
