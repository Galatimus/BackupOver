...
workbook = xlsxwriter.Workbook(u'Nkso.xlsx')
...
class Cian_Zem(Spider):
     ...
     def task_generator(self):
	  for x in range(1,71):
               yield Task ('post',url='http://mb.nkso.ru/?PAGEN_1=%d'%x+'&SIZEN_1=30',network_try_count=100)
     ...
     def task_post(self,grab,task):
	  for elem in grab.doc.select('//div[@id="data_content"]/a'):
     ...
     def task_item(self, grab, task):
	  ...
	  try:
	       ...
	       ray = ''          
	  try:
	       punkt= grab.doc.select(u'//tr[@id="rabotodatel_lines0"]/td[1]/text()[1]').text()#.split('Должность: ')[1].split(' Подробнее')[0]
	  except IndexError:
	       punkt = ''
	  ...
	  try:
	       ...
	       #for m in grab.doc.select(u'//td[contains(text(),"Квалификационный аттестат")]/following-sibling::td/b[contains(text(),"Направление:")]/following-sibling::text()[1]'):
		    ...
		    #ln.append(mes)
	       #ter = ', '.join(ln)
	       ter= grab.doc.select(u'//b[contains(text(),"Направление оценочной деятельности")]/following::tbody[1]/tr/td[4]').text()
	  except IndexError:
	       ter =''
	  ...
	  try:
	       ...
	       uliza = ''
	  try:
	       dom = grab.doc.select(u'//b[contains(text(),"Номера контактных телефонов:")]/following::td[1]/p').text()#.split(', ')[0]
	  except IndexError:
	       dom = ''       
	  try:
	       ...
	       trassa = ''       
	  try:
	       udal = grab.doc.select(u'//b[contains(text(),"Почтовый адрес:")]/following::td[1]/p').text().split(', ')[1].replace(u'г.','')
	  except IndexError:
	       udal = ''
	       
	  try:
	       price = grab.doc.select(u'//b[contains(text(),"Состояние членства:")]/following::td[1]/p').text()#.split(u'Цена - ')[1]
	  except IndexError:
	       price = ''   
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write(self.result, 11, u'Ассоциация СРО «НКСО»')
