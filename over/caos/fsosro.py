...
workbook = xlsxwriter.Workbook(u'Fsosro.xlsx')
...
class Cian_Zem(Spider):
     ...
     def task_generator(self):
	  #for x in range(1,28):
          yield Task ('post',url='http://api.fsosro.ru/api/app/member_list_app.aspx',refresh_cache=True,network_try_count=100)
     ...
     def task_post(self,grab,task):
	  for elem in grab.doc.select('//a[@class="mlink"]'):
	       ur = 'http://api.fsosro.ru/api/app/member_card_app.aspx?id='+elem.attr('data-id')
	       #print ur
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write(self.result, 11, u'СРО «СФСО»')
