...
try:
    database = pymssql.connect(server="109.68.190.240",port = 33306,user='sa',password='parsing2018+',database='TestDB',charset='cp1251')
    print 'Connect to MSSQL Server!!'
...
#cursor.execute('CREATE TABLE persons(id INT, name VARCHAR(100))')
#cursor.execute("INSERT INTO persons VALUES(1, 'John Doe')")
#cursor.execute("INSERT INTO persons VALUES(2, 'Jane Doe')")

#cursor.execute('CREATE TABLE com(SUBJECT VARCHAR(255),MUN_WARD VARCHAR(255),CITY VARCHAR(255),VGT VARCHAR(255),STREET VARCHAR(255),HOUSE VARCHAR(255),LANDMARK VARCHAR(255), SEGMENT VARCHAR(255),             OBJ_TYPE VARCHAR(255),
                                #OBJ_USE VARCHAR(255),  
                                #KLASS VARCHAR(255),
                                #PRICE VARCHAR(255)')
