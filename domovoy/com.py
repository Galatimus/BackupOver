...
workbook = xlsxwriter.Workbook(u'0001-0024_00_C_001-0022_DOMV45.xlsx')
...
class Cian_Zem(Spider):
     ...
     def task_generator(self):
	  for x in range(1,5):
               yield Task ('post',url='http://domovoy45.ru/kommercheskaya?&page=%d'%x,network_try_count=100)
	       
	       
     def task_post(self,grab,task):
	  for elem in grab.doc.select('//div[@class="rg-img"]/a'):
     ...
     def task_item(self, grab, task):
	  try:
	       ...
	       sub = u'Курганская область'#reduce(lambda dt, r: dt.replace(r[0], r[1]), conv, dt)
	  except DataNotFound:
	       sub = ''
	  try:
	  ...
	  try:
	       ...
	       punkt= u'Курган'#grab.doc.select(u'//div[contains(text(),"Город:")]/span').text()#.split(', ')[2]
	  ...
	  try:
	       ...
	       ter= grab.doc.select(u'//label[contains(text(),"Район:")]/following-sibling::span').text()#.split(', ')[3].replace(u'улица','')
	       #else:
	  ...
	  try:
	       ...
	       uliza = grab.doc.select(u'//label[contains(text(),"Улица:")]/following-sibling::span').text().split(', ')[0]
	       #else:
		    #uliza = ''
	  except IndexError:
	       uliza = ''
	  try:
	       dom = grab.doc.select(u'//label[contains(text(),"Улица:")]/following-sibling::span').number()
	  except IndexError:
	       dom = ''       
	  try:
	  ...
	  try:
	       price = grab.doc.select(u'//h3').text().split(u'Цена - ')[1]
	  except DataNotFound:
	       price = ''   
	  try:
	       plosh = grab.doc.select(u'//label[contains(text(),"Площадь:")]/following-sibling::span').text()
	  except DataNotFound:
	       plosh = ''
	  ...
	  try:
	       opis = grab.doc.select(u'//div[@class="row-fluid"]/p').text()#.replace(u'Описание','')  
	  except IndexError:
	       opis = ''
	       
	  try:
	       #try: 
	       phone = re.sub('[^\d\,]', u'',grab.doc.select(u'//div[@class="rv_phone phonenr"]').text())
	       #except IndexError:
	  ...
	  try:
	       comp = grab.doc.select(u'//div[@class="avtor"]/a[contains(@href, "firma")]/h4').text()#.split(' :: ')[0]
	  except IndexError:
	       comp = ''
	  ...
	  try:
	       ...
	       dt= grab.doc.select(u'//div[@class="advdesc"]').text().split(u' Опубликовано: ')[1]
	  ...
	  projects = {'url': task.url,
	              ...
	              'mat': mat,
	              'god':godp,
                      'vid': vid.replace(u'сегодня',datetime.today().strftime('%d.%m.%Y')[:10]),
                      'ohrana':ohrana,
                      'gaz': gaz,
                      'voda': voda,
                      'kanaliz': kanal,
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write(self.result, 26, u'Домовой45')
