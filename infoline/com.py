...
workbook = xlsxwriter.Workbook(u'0001-0081_00_C_001-0036_VRX-RU.xlsx')
...
class Cian_Zem(Spider):
     ...
     def task_generator(self):
	  for x in range(1,53):#30
               yield Task ('post',url='http://www.vrx.ru/data/prodazha/kommercheskay_nedvizhimost.html?pg=%d'%x,network_try_count=100)
	  for x1 in range(1,33):#18
               yield Task ('post',url='http://www.vrx.ru/data/arenda/kommercheskay_nedvizhimost.html?pg=%d'%x1,network_try_count=100)
     ...
     def task_post(self,grab,task):
	  for elem in grab.doc.select(u'//div[@class="adr"]/a'):
