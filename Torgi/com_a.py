...
workbook = xlsxwriter.Workbook(u'0001-0057_00_C_001-0220_TORGI_Аренда.xlsx')
...
class Torgi_Zem(Spider):
     ...
     def prepare(self):
	  ...
	  #self.f = page
	  #self.link =l[i]
	  self.ws = workbook.add_worksheet(u'Torgi_Коммерческая')
	  ...
	  self.ws.write(0, 11, u"Площадь")
	  self.ws.write(0, 12, u"Срок заключения договора")
     ...
     def task_generator(self):
	  l= open('links/Torgi_Com_arenda.txt').read().splitlines()
	  self.dc = len(l)
	  print self.dc
	  for line in l:
     ...
     def task_item(self, grab, task):
	  ...
	  try:
	       try:
		    ...
		    stat = grab.doc.select(u'//label[contains(text(),"Статус лота:")]/following::span[1]').text()
	  except IndexError:
	       stat = ''
	  try:
	  ...
	  try:
	       ...
	       strana = ''
	  try:
	       sub = grab.doc.select(u'//label[contains(text(),"Местоположение имущества:")]/following::span[1]').text().split(', ')[0]
	  except IndexError:
	       sub = ''
	  try:
	       mesto = grab.doc.select(u'//label[contains(text(),"Местоположение имущества:")]/following::span[1]').text()
	  except IndexError:
	       mesto = ''
	  try:
	       mesto_det = grab.doc.select(u'//label[contains(text(),"Детальное местоположение имущества:")]/following::span[1]').text()
	  except IndexError:
	       mesto_det = ''
	  try:
	       plosh = grab.doc.select(u'//label[contains(text(),"Площадь")]/following::span[1]').text()+u' м2'
	  except IndexError:
	       plosh =''
	  try:
	       srok_dog = grab.doc.select(u'//label[contains(text(),"Срок заключения договора:")]/following::td[1]').text()
	  except IndexError:
	       srok_dog =''
	  try:
	  ...
	  try:
	       ...
	       data_iz = ''
	  try:
	       data_ok =  grab.doc.select(u'//label[contains(text(),"Дата окончания приема заявок")]/following::span[1]').text()
	  except IndexError:
	       data_ok = ''
          try:
               ...
               data_otm = ''
          try:
               pobeda =  grab.doc.select(u'//label[contains(text(),"Победитель торгов")]/following::span[1]').text()
          except IndexError:
               pobeda = ''
...
time.sleep(2)
print('Save it...')    
#command = 'mount -a'# cifs //192.168.1.6/e /home/oleg/Pars -o username=oleg,password=1122,_netdev,rw,iocharset=utf8,file_mode=0777,dir_mode=0777' #'mount -a -O _netdev'
