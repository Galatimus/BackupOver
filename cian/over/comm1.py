...
while True:
	...
	def prepare(self):
	    self.f = page
	    self.workbook = xlsxwriter.Workbook(u'com/Cian_'+str(i+1)+'.xlsx')
	...
	def task_item(self, grab, task):
	    ...
	    try:
		sub = grab.doc.select(u'//address[@class="address--D3O4n"]').text().split(', ')[0]
	    except IndexError:
		sub = ''	
	    try:
		usl = grab.doc.select(u'//p[@class="description--N64UT"]').text()
	    except IndexError:
		usl = ''	
	    try:
		ray = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"р-н ")]').text()
	    except IndexError:
		ray =''
	    try:
		if sub == u'Москва':
		    ...
		    if  grab.doc.select(u'//address[@class="address--D3O4n"]/a[2][contains(text(),"р-н ")]').exists()==True:
			punkt= grab.doc.select(u'//address[@class="address--D3O4n"]/a[3]').text()
		    elif grab.doc.select(u'//address[@class="address--D3O4n"]/a[3][contains(text(),"р-н ")]').exists()==True:
			punkt= grab.doc.select(u'//address[@class="address--D3O4n"]/a[2]').text()
		    else:
			punkt=grab.doc.select(u'//address[@class="address--D3O4n"]/a[2]').text()
	    except IndexError:
		punkt = ''
	    try:
		ter=  grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"городской округ")]').text()
	    except IndexError:
		ter =''
	    try:
		try:
			...
			try:
				...
				try:
				    try:
					try:
					    uliza = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"ул.")]').text()
					except IndexError:
					    uliza = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"пер.")]').text()
				    except IndexError:
					uliza = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"просп.")]').text()
				except IndexError:
				    uliza = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"ш.")]').text()
			    except IndexError:
				uliza = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"бул.")]').text()
			except IndexError:
			    uliza = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"проезд")]').text()
		    except IndexError:
			uliza = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"наб.")]').text()
		except IndexError:
		    uliza = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(text(),"пл.")]').text()
	    except IndexError:
		uliza =''
		...
		if uliza == '':
		    ...
		    dom = grab.doc.select(u'//address[@class="address--D3O4n"]/a[contains(@href,"house")]').text()
	    except IndexError:
		dom = ''
	    ...
	    try:
		naz = grab.doc.select(u'//h1').text().split(', ')[0]
	      #print naz
	    except IndexError:
		naz = '' 
	    ...
	    try:
		price = grab.doc.select(u'//div[@class="price--xQiE6"]').text()
	      #print price
	    except IndexError:
		price = ''
	    ...
	    try:
		et = grab.doc.select(u'//div[contains(text(),"Этаж")]/following-sibling::div[1]').text().split(u' из ')[0]
	    except IndexError:
		et = ''
		
	    try:
		et2 = grab.doc.select(u'//div[contains(text(),"Этаж")]/following-sibling::div[1]').text().split(u' из ')[1]
	    except IndexError:
		et2 = ''
	    ...
	    try:
		try:
		    lico = grab.doc.select(u'//div[@class="link-wrapper--EDwrd"]/a[contains(@href,"agents")][1]/h2').text()
		except IndexError:
	    ...
	    try:
		try:
		    comp = grab.doc.select(u'//div[@class="link-wrapper--EDwrd"]/a[contains(@href,"company")][1]/h2').text()
		except IndexError:
		...
		ohrana =''
	    try:
		gaz = grab.doc.select(u'//div[@class="address--2T-DP"]').text()
	    except IndexError:
		gaz =''
	    try:
		voda =  grab.doc.select(u'//div[@class="underground--1fVk6"]/a').text()
	    except IndexError:
		voda =''
	    try:
		kanal = grab.doc.select(u'//div[@class="underground--1fVk6"]/span').text().replace(', ','')
	    except IndexError:
		kanal =''
	    try:
