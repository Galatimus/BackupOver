...
l= open('links/aren.txt').read().splitlines()
s = ['offers/spaces/','offers/spaces/sale/']
     ...
     for z in range(len(l)):
	  ...
	  class move_Com(Spider): 
	       def prepare(self):
		    ...
		    for p in range(1,30):
			 try:
			      ...
			      g.go(self.page)
			      self.num = re.sub('[^\d]','',g.doc.select(u'//span[@class="counter-info"]').text())
		    ...
		    self.workbook = xlsxwriter.Workbook(u'comm/Arendator_'+str(w+1)+'_'+str(z+1)+'.xlsx')
	       ...
	       def task_generator(self):
		    if self.pag == 1:
			 yield Task ('post',url=self.page,refresh_cache=True,network_try_count=100)
		    else:
			 for x in range(self.pag+1):
			      yield Task ('post',url=self.page+'page:'+ str(x)+'/',refresh_cache=True,network_try_count=100)
