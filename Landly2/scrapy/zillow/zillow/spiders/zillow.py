...
class ZillowSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.zillow.com/homes/'+links.split('-')[-1][:-1] for links in open('city_zip.txt').read().splitlines()] 
    #start_urls = [links for links in open('city_zip.txt').read().splitlines()]
    ...
    def parse_product(self, response): 
        ...
        history_date = response.xpath('//div[contains(text(),"Price history")]/text()').extract()
        
        try:    
            item['pricehistory'] = [{'Date': history_date,'Price': response.xpath('//tr[@class="zsg-table_interactive"]/td[3]/text()').getall(),'Event': response.xpath('//tr[@class="zsg-table_interactive"]/td[2]/text()').getall()}]
        except (IndexError,AttributeError):
            item['pricehistory'] = ''
